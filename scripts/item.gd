...
func spawn_item(x, y, sprite: Texture2D, item_name: String, item_label: String):
	...
	# Set the item's sprite texture
	var sprite_node = item_instance.get_node("Sprite2D")
	if sprite_node and sprite is Texture2D:
		sprite_node.texture = sprite
	
	# Get the Label node FROM THE ITEM SCENE and set its text
	var label_node = item_instance.get_node("CanvasLayer/TextureRect/Label")
	if label_node:
		label_node.text = item_label
	
	# Add the item to the main scene
	get_tree().current_scene.add_child(item_instance)

func _ready():
	# Get the popup menu using the correct path from your scene tree
	var popup_menu = $CanvasLayer/Window/Popup/PopupMenu
	if popup_menu:
		# When an item in the popup is pressed, the '_on_popup_id_pressed' function will be called.
		popup_menu.id_pressed.connect(_on_popup_id_pressed)
	else:
		print("Error: PopupMenu node not found at path 'CanvasLayer/Window/Popup/PopupMenu'.")
		
	# Hide the popup container by default so it's not visible on game start
...
func _show_popup(prompt: String, pos_rel: String, options: Array[String]):
	...
	"""
	# Use the correct paths from your scene tree
	var popup_container = $CanvasLayer/Window/Popup
	var popup_menu = $CanvasLayer/Window/Popup/PopupMenu
